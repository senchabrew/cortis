...
namespace ProtoHandlerGen
{
    public sealed partial class ProtoHandlerGenerator
    {
        ...
        /// <summary>
        /// using 宣言を出力する。InfrastructureNamespace は属性の定義位置から導出されるため
        /// 固定の using と一致しうる。重複した using は CS0105 になるので取り除く。
        /// </summary>
        static void EmitUsings(StringBuilder sb, PresenterModel model)
        {
            var namespaces = new List<string> { "System", "System.CodeDom.Compiler" };

            if (model.InfrastructureNamespace != null)
                namespaces.Add(model.InfrastructureNamespace);

            namespaces.Add("Cortis");
            namespaces.Add("R3");
            namespaces.Add("VContainer");
            namespaces.Add("VContainer.Unity");

            if (model.Handlers.Any(h => h.IsAsync))
                namespaces.Add("Cysharp.Threading.Tasks");
